...
def read_camera(queue_camera_bm, queue_camera_full,queue_camera_show):
    ...
    #camera2.set(4, 360)
    ##camera1.set(3, 1280)  # 设置分辨率
    ##camera1.set(4, 720)
    ##camera2.set(3, 1280)  # 设置分辨率
    ##camera2.set(4, 720)
    ...
    while True:
        ...
        Left = cv2.imread('D:/img/img_190617/img_q/left_5.bmp')
        Right = cv2.imread('D:/img/img_190617/img_q/right_5.bmp')
        if pickOneOfTwo == 0:    #差值取图像
            ...
            continue
        if ret1 and ret2:
            CutImg1 = Left[180:540, :]  # y x
            CutImg2 = Right[197:557, :]  # y x
            ##cv2.imshow("left", CutImg1)
            ##cv2.imshow("right", CutImg2)
            cv2.waitKey(1)
...
def showImg(queue_camera_show, bm_data, Object_data):
    ...
    zeroimg = np.zeros((720,1280,3), dtype=np.uint8)
    #DeepData  = np.zeros((42), dtype=np.uint16)
    DeepData  = np.zeros((128), dtype=np.uint16)
...
def BM(queue_camera_bm, bm_data):
    ...
    cv2.resizeWindow("Noise reduction", 640, 180);
    ...
    cv2.resizeWindow("Sort by distance", 640, 180);
    ...
    while True:
        ...
        cut_threeD = threeD[0:360:5, 0:1280:10, 2]
        ...
        cut_data = cut_threeD.reshape(9216, 1)  # (462,1)  实时图像数据
        ...
        cut_data_out = cut_data_out.reshape(72, 128)  # 重新整形

        cut_data_show = cut_data_out.copy()  # 结果显示
        cut_data_show = cut_data_show / 5.88
        cut_data_show = cut_data_show.astype(np.uint8)
        #kernel = np.ones((2, 2), np.uint8)
        #cut_data_show = cv2.dilate(cut_data_show, kernel)  
        ...
        deep_data = deep_data.reshape(128)
