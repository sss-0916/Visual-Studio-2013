...
def broadcast(bc_recv, people_recv):
    ...
    while True:
        for i in range(4):
            ...
            try:
                #people_flag = people_recv.get_nowait()
            ...
            #if a <= 0 and i != 2 and people_flag == False:
            #    continue
...
def read_camera(queue_camera_bm, queue_camera_full, queue_camera_show):
    ...
    while True:
        ...
        if ret1 and ret2:
            ...
            ##CutImg1 = cv2.imread("D:/img/img_up/left_3.bmp")
            ##CutImg2 = cv2.imread("D:/img/img_up/right_3.bmp")
            ##frame3 = cv2.imread("D:/img/img_up/up_3.bmp")
        ...
        #frame3 = cv2.imread("D:/img/img_up/up_1.bmp")
...
def showImg(queue_camera_show, bm_data, Object_data, bc_send, bg_recv, people_detection):
    ...
    while True:         
        ...
        if cnt > 20:
            distance.sort()
            #bc_send.send(distance[10])
            try:
                bc_send.put(distance[int(len(distance) / 2)])
            except:
                bc_send.put(0)
                pass
            #bc_send.put(distance)
            #people_detection.put(person_sige)
            distance.clear()
            cnt = 0

            print(people_cnt)

            # 20帧图象中超过5帧出现行人框，代表有行人
            if people_cnt > 4:
                people_detection.put(True)
            else:
                people_detection.put(False)
                #print(people_detection.qsize())
            people_cnt = 0
        ...
        cv2.waitKey(10)
        #end = time.time()
        #end = datetime.datetime.microsecond()
