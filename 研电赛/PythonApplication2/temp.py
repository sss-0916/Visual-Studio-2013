...
import camera_configs_3 as camera_configs
import math
...
cv2.moveWindow("right", 0, 380)
cv2.resizeWindow("left", 1280, 720)
cv2.resizeWindow("right", 1280, 720)
cv2.createTrackbar("num", "depth", 0, 10, lambda x: None)
cv2.createTrackbar("blockSize", "depth", 16, 255, lambda x: None)
...
#for y in range(36):
#    ground_filter[y + 36] -= 200

# 300 ~ 360高度提高200
#for y in range(12):
#    ground_filter[y + 60] -= 200
...
# 数字降噪
digital_noise_reduction = np.zeros((256 * 72, 3), dtype = np.int16)
...
while True:
    ...
    # 摄像头交换
    if camera_swap_flag:
        ret1, temp_frame1 = camera2.read()
        ret2, temp_frame2 = camera1.read()
    else:
        ret1, temp_frame1 = camera1.read()
        ret2, temp_frame2 = camera2.read()

    if not ret1 or not ret2:
        break

    #frame1 = cv2.imread('D:/img/img_q/left_5.bmp')
    #frame2 = cv2.imread('D:/img/img_q/right_5.bmp')
    #temp_left_frame1 = cv2.imread('D:/img/img_190617/img_q/left_5.bmp')
    #temp_right_frame2 = cv2.imread('D:/img/img_190617/img_q/right_5.bmp')

    #frame2 = temp_left_frame1[197 : 557, :]
    #frame1 = temp_left_frame1[180 : 540, :]
    #frame1 = temp_right_frame2[180 : 540, :]
    frame1 = temp_frame1[180 : 540, :]
    frame2 = temp_frame2[197 : 557, :]

    #frame1 = cv2.imread('D:/img/img_ground2/left_0.bmp')
    #frame2 = cv2.imread('D:/img/img_ground2/right_0.bmp')

    #frame1 = cv2.imread('D:/img/img_ground/left_3.bmp')
    #frame2 = cv2.imread('D:/img/img_ground/right_3.bmp')

    #frame1 = cv2.imread('D:/img/img_people/left_0.bmp')
    #frame2 = cv2.imread('D:/img/img_people/right_0.bmp')
    ...
    #src_threeD = threeD[0 : 360 : 5, 0 : 1280 : 5, :]
    #src_threeD = src_threeD / 10
    #src_threeD.astype(np.int16)

    # 对threeD进行间隔5采样
    sampling_threeD = threeD[0 : 360 : 5, 0 : 1280 : 5, 2]
    ...
    # cv2.imwrite('interalSamping.jpg', sampling_result)
    cv2.imshow("interval sampling", sampling_result)

    # 数字降噪
    dnr_temp = sampling_threeD.reshape(256 * 72, 1)
    # 剔除过期数据
    digital_noise_reduction = digital_noise_reduction[:, 0 : 2]
    # 引入最新一帧
    digital_noise_reduction = np.concatenate((dnr_temp, digital_noise_reduction), axis = 1)
    ...
    dnr_result = dnr_result.reshape(72, 256)
    ...
    #dnr_result_show = cv2.dilate(dnr_result_show, kernel)  # 膨胀
    #cv2.imwrite('digitalNoice.jpg', dnr_result_show)
    ...
    distance_result = dnr_result[0, :]
    distance_result = distance_result.reshape(256)
    ...
    #rectangle = np.zeros([120, 160, 3], np.uint8)
    #cv2.rectangle(rectangle, (24, 36), (108, 120), (255, 255, 255), -1)
    #cv2.imshow("image", rectangle)

    #bitwiseAnd = cv2.bitwise_and(rectangle, dnr_result)
    #cv2.imshow("AND", bitwiseAnd)

    #for i in range(160):
    #    for j in range(120):
    #        if distance_result[i] * 10 == src_threeD[j, i, 2] and src_threeD[j, i, 2] != 0:
    #            y = abs(src_threeD[j, i, 1])
    #            z = abs(src_threeD[j, i, 2])
                #print(src_threeD)
                #print(z)
                #print(y)
                #angle = 45 - math.atan(y / z) * 180 / math.pi
                #print(angle)
                #hypotenuse = math.sqrt(y * y + z * z)
                #continue
            #else:
            #    distance_result[i] = 0
    ...
    #cv2.imwrite("left.bmp", img1_rectified)
    #cv2.imwrite("right.bmp", img2_rectified)
